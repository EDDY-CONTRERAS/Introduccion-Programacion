...
void Lectura(int, int);  /*Prototipos de funciiones. */
...
void main(void)
{
    ...
    if (RES)
        /*Si RES tiene un valor verdadero -diferente de 0-, se escribe
        la posicion en la que se encontro el elemento. */
...
/* Esta funcion localiza en el arreglo un elemento determinado. Si el elemento es encontrado, regresa la posicion corresondiente. En caso contrario, regresa 0. */
{
    int I = 0, BAN = 0, RES;
    while (I < T && !BAN)
        if (A[I] == K)
